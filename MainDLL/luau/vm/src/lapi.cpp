 ...
 */
std::map<uintptr_t, bool> CCLOSURE_MADE{};
std::map<uintptr_t, bool> get_closures_made()
{
    return CCLOSURE_MADE;
}
...
void lua_pushcclosurek(lua_State* L, lua_CFunction fn, const char* debugname, int nup, lua_Continuation cont)
{
    ...
    api_incr_top(L);
    CCLOSURE_MADE[reinterpret_cast<int>(fn)] = true;
}
...
void lua_clonefunction(lua_State* L, int idx)
{
    ...
    if (cl->isC) {
        ...
        //const char* debugname = cl->c.debugname; //newcl->c.cont = cl->c.cont;
        newcl->c.f = func;
        //newcl->c.debugname = debugname;
