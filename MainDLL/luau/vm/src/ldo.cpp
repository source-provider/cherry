...
int luaD_rawrunprotected(lua_State* L, Pfunc f, void* ud)
{
    ...
    {
        //std::cout << "[MASSIVE LUA EXCEPTION 1]: " << e.what() << "\n";
    ...
    catch (std::exception& e)
    {
        ...
        try
        {
           ...
           // std::cout << "[MASSIVE LUA EXCEPTION 2]: " << e.what() << "\n";
...
l_noret luaD_throw(lua_State* L, int errcode)
{
    //cherry::offsets::execution::luaD_throw((int)L, errcode);
...
void luaD_call(lua_State* L, StkId func, int nResults)
{
    ...
    {                                        // is a Lua function?
        ...
        cherry::offsets::execution::luau_execute((int)L);
...
auto task_scheduler = cherry::scheduler::get_singleton();
...
static void resume(lua_State* L, void* ud)
{
    ...
    else
    {
        ...
        if (cl->isC)
        {
            ...
            if (!cl->c.cont)
            {
                task_scheduler->push([L, firstArg](DWORD)
                    {
...
{
    task_scheduler->push([L, status](DWORD)
        {
            L->nCcalls = L->baseCcalls;
            L->isactive = false;
...
{
    task_scheduler->push([L, from, nargs](DWORD)
        {
            int status;
