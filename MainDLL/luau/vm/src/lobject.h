...
LUAVM_SHUFFLE8(; ,
   member_enc<TValue*> k,
   member_enc<Instruction*> code,
   member_enc<struct Proto**> p,
   member_enc<uint8_t*> lineinfo,
   member_enc<int*> abslineinfo,
   member_enc<struct LocVar*> locvars,
   member_enc<TString**> upvalues,
   member_enc<TString*> source
);

LUAVM_SHUFFLE2(; ,
    debugname_enc<TString*> debugname,
    debuginsn_enc<uint8_t*> debuginsn
);
...
#define luaO_nilobject (TValue*)cherry::offsets::lua_nilobject
