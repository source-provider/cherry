...
void luaG_breakpoint(lua_State* L, Proto* p, int line, bool enable)
{
    if (p->lineinfo)
    {
        for (int i = 0; i < p->sizecode; ++i)
        {
            ...
            if (LUAU_INSN_OP(p->code[i]) == ((LOP_PREPVARARGS * 227) & 0xFF))
                continue;
            ...
            uint8_t op = enable ? ((LOP_BREAK * 227) & 0xFF)  : LUAU_INSN_OP(p->debuginsn[i]);
...
bool luaG_onbreak(lua_State* L)
{
    ...
    return LUAU_INSN_OP(*L->ci->savedpc) == ((LOP_BREAK * 227) & 0xFF);
}
...
static void getcoverage(Proto* p, int depth, int* buffer, size_t size, void* context, lua_Coverage callback)
{
    ...
    for (int i = 0; i < p->sizecode; ++i)
    {
        ...
        if (LUAU_INSN_OP(insn) != ((LOP_COVERAGE * 227) & 0xFF))
            continue;
